-- 1) strict chain: ALL previous groups in the category must be passed
...
BEGIN
  ...
  SELECT count(*) INTO v_missing
   ...
   WHERE g.category = v_cat
     AND g.order_index < v_order
     AND NOT EXISTS (
       SELECT 1 FROM public.user_group_progress p
        WHERE p.user_id = _user_id AND p.group_id = g.id AND p.passed
     );
...
-- 2) helper: level is locked when its group is locked
CREATE OR REPLACE FUNCTION public.is_level_unlocked(_user_id uuid, _level_id uuid)
RETURNS boolean
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  v_group uuid;
BEGIN
  SELECT group_id INTO v_group FROM public.levels WHERE id = _level_id;
  IF NOT FOUND THEN
    RETURN false;
  END IF;
  IF v_group IS NULL THEN
    RETURN true;
  END IF;
  RETURN public.is_group_unlocked(_user_id, v_group);
END;
$function$;

GRANT EXECUTE ON FUNCTION public.is_level_unlocked(uuid, uuid) TO authenticated;

-- 3) guard practice questions
...
BEGIN
  ...
  IF NOT v_is_admin THEN
    FOREACH v_lid IN ARRAY COALESCE(p_level_ids, ARRAY[]::uuid[]) LOOP
      IF NOT public.is_level_unlocked(auth.uid(), v_lid) THEN
        RAISE EXCEPTION 'Level locked';
      END IF;
    END LOOP;
  END IF;
...
-- 4) guard level test fetch
CREATE OR REPLACE FUNCTION public.get_level_test(p_level_id uuid, p_lang text DEFAULT 'cs'::text)
RETURNS jsonb
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  IF NOT public.has_role(auth.uid(), 'admin')
     AND NOT public.is_level_unlocked(auth.uid(), p_level_id) THEN
    RAISE EXCEPTION 'Level locked';
  END IF;

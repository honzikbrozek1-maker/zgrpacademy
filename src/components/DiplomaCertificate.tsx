  ...
  const handlePrint = () => {
    ...
    win.document.write(`
      ...
      <style>
        ...
        .safe { position: absolute; inset: 18mm 22mm 18mm 22mm; display: flex; flex-direction: column; align-items: center; text-align: center; }
        .logo { width: 90px; height: auto; margin-bottom: 6px; }
        .title { font-family: 'Cormorant Garamond', 'Times New Roman', serif; font-size: 64px; letter-spacing: 6px; margin: 4px 0 6px; font-weight: 500; }
        .udelen { font-size: 14px; letter-spacing: 6px; color: #555; margin-bottom: 14px; }
        .recipient { font-family: 'Cormorant Garamond', serif; font-size: 30px; font-weight: 600; margin: 4px 0 10px; }
        .divider { width: 45%; height: 2px; background: #1a1a1a; margin: 8px 0 18px; }
        .italic { font-family: 'Cormorant Garamond', serif; font-style: italic; font-weight: 600; font-size: 19px; line-height: 1.4; color: #111; max-width: 130mm; margin: 0 auto; white-space: pre-line; }
        .body { font-size: 14px; line-height: 1.6; color: #2a2a2a; max-width: 130mm; margin: 18px auto 0; }
        .score { font-size: 32px; font-weight: 700; color: hsl(168, 65%, 38%); margin: 16px 0 4px; }
        .meta { font-size: 13px; color: #444; margin-top: 4px; }
        .meta strong { color: #111; }
        .sub { font-size: 12px; color: #666; margin-top: 6px; letter-spacing: 1px; }
      ...
      <div class="page">
        ...
        <div class="safe">
          ...
          <div class="meta">Datum vydání: <strong>${safe(fmtDate(issued))}</strong></div>
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        />
        {/* Safe area kept well inside the gold border */}
        <div className="absolute inset-[9%_12%_8%_12%] flex flex-col items-center text-center text-foreground">
          <img src={logoSpolek} alt="Spolek v Rovnováze z.s." className="h-14 w-auto mb-1" />
          <h1
            className="text-[clamp(28px,6.2vw,58px)] tracking-[0.18em] font-medium leading-none mt-1"
            style={{ fontFamily: '"Cormorant Garamond", "Times New Roman", serif' }}
          >
            {title.toUpperCase()}
          </h1>
          <div className="text-[clamp(9px,1.2vw,12px)] tracking-[0.4em] text-muted-foreground mt-2 mb-3">
            UDĚLEN PRO
          </div>
          <div
            className="font-semibold text-[clamp(18px,3vw,28px)]"
            style={{ fontFamily: '"Cormorant Garamond", serif' }}
          >
            {userName}
          </div>
          <div className="w-[45%] h-[2px] bg-foreground my-3" />
          <p
            className="italic font-semibold text-[clamp(13px,1.9vw,18px)] leading-snug max-w-[78%] text-foreground"
            style={{ fontFamily: '"Cormorant Garamond", serif' }}
          >
            {groupTitle}
          </p>
          {bodyText && (
            <p className="text-[clamp(11px,1.4vw,14px)] leading-relaxed max-w-[78%] text-foreground/85 mt-3 whitespace-pre-line">
              {bodyText}
            </p>
          )}
          <div className="text-[clamp(20px,3.6vw,30px)] font-bold text-primary mt-3">{score}%</div>
          <div className="text-[clamp(10px,1.3vw,13px)] text-muted-foreground mt-1 space-y-0.5">
          ...
          <div className="mt-auto pt-3 w-full flex flex-col items-center">
            <div className="w-[55%] border-t border-foreground/50 mb-1" />
